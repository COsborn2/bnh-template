...
try {
  ...
  // Explicit exit: postgres-js can keep the event loop alive after
  // migrations complete, so don't rely on a natural exit here.
  // process.exit(1) in the catch above terminates immediately, so this
  // finally block only runs on the success path.
  await client.end();
  process.exit(0);
}
